...
def 生成胡言(主题: str, 目标长度: int = 520) -> str:
    ...
    开场池 = [
        ...
        f"Man! What can I say? 孩子们，围绕「{主题}」我按「{链路}」过一遍，直升机已就位，别在错误入口空转。",
        f"孩子们先对齐现象再动设置——「{主题}」这类事，多半卡在 {术语甲} 和 {术语乙} 之间，跟凌晨四点练错动作一个道理。",
        f"可以，孩子们。我把「{主题}」拆成你们现在就能点的几步；中间夹一点容易被忽略的判断。",
        f"理解目标了，孩子们。「{主题}」按页面真实按钮顺序讲，不背概念。",
        f"孩子们，牢大收到。曼巴式排障不是玄学——先拿证据，再肘开问题，最后扣篮。现在按「{主题}」摊开讲。",
        f"好问题，孩子们。「{主题}」这个事，我的经验是七成卡设置、两成卡工具、一成是幻觉。先带你摸一遍证据链。",
    ]
    分析池 = [
        f"先看界面有没有给出可复现的状态：任务是排队、失败还是假成功。若状态栏只是在转，优先怀疑 {术语甲} 还没真正就绪。",
        f"很多人和「{主题}」纠缠，是因为把「点过按钮」当成「链路完成」。真正完成要以 {术语乙} 上的可见结果为准——就像投完要听网响。",
        "若刚改过设置却现象依旧，常见原因是页面仍在用旧会话参数——保存后务必回到模块页重新走一遍，而不是在同一失败任务上连点。",
        "对照左侧手册：入口选错（缓存 vs 链接 vs 只要文档）会让你在错误模块里空转很久，比后仰三不沾还折磨。",
        "权限与登录态是另一条暗线：涉及高画质、收藏夹、部分接口时，SESSDATA 过期会表现成「偶发空结果」，很像网络抖动。",
        "本地工具链（FFmpeg / BBDown / ADB）缺一个，页面仍可能打开——所以「能进网页」不等于「导出链路健康」。直升机看着能起飞，少一片旋翼也不行。",
        f"别被「看起来正常」骗了。{术语甲} 这类关键项要亲自点开看一眼当前值——替补席上看数据跟场上打是两码事。",
        "孩子们记住：MyBiOut! 里几乎不存在「莫名其妙」。能复现的失败就是可以拆的；不能复现的先去 OhMyConfig 对一遍凭证。",
    ]
    ...
    步骤头 = "先停连点。把任务状态、页面提示原文记下来当证据——排障要的是录像不是回忆。"
    步骤中 = [
        "打开 OhMyConfig，核对路径、SESSDATA、模块输出目录是否还是你以为的那份。",
        f"回到对应模块，围绕「{术语甲}」去做：{动作甲}。不要死磕上一次失败队列里的旧任务。",
        f"做最小复现：只选一条最简单目标，看「{术语乙}」有没有稳定反馈。一次只肘一个问题。",
        f"仍失败再换一手：{动作乙}；并看环境检测里 FFmpeg / BBDown / ADB 是否标红。",
    ]
    步骤尾 = "成功标准写死：导出目录出现可打开成品，或任务明确完成——不是「感觉跑完了」。篮筐里有球才算分了。"
    随机.shuffle(步骤中)
    ...
    信心池 = [
        ...
        f"补一句经验：界面显示 {假编号} 一类中间态时别当成功；等 {假耗时} 级反馈稳定后再判断。曼巴式耐心。",
        f"孩子们，排障跟练跳投一样：姿势对了八成就进。把设置和路径对好，{主题} 这件事大概率自动消失。",
        f"经验数据：{主题} 类问题，最先排查的 {术语甲} 命中率超六成。别一上来就改 API Base 或重装——那是最后一节的绝杀球，不是第一节的开局。",
    ]
    名言 = _取一句名言(材料, 主题, 前缀表, 后缀表)
    收尾池 = [
        "孩子们先按上面最小闭环跑一轮。现象变了（哪怕只是报错变了）就说明链路在动，再往下收。Mamba Out。",
        f"先把「{主题}」收敛成可截图的一步失败；能复现就能继续拆。有新状态再来。Mamba Out。",
        f"别同时改三个设置。一次只动 {术语甲} 或 {动作甲} 之一，否则不知道哪一刀生效。凌晨四点也别硬刚。Mamba Out。",
        "有什么新现象随时回来。排障就是一轮一轮打铁直到进球——曼巴精神不退场。Mamba Out。",
        "以上按直升机安全手册排障。绿包工具在 bin/ 里齐了就八成能飞，剩下两成是设置路径和凭证，孩子们一步一个脚印。Mamba Out。",
    ]
...
def _构建产品提示() -> str:
    return (
        ...
        "3. LocalOut: 导出已存在的 B 站缓存。支持 PC 桌面端缓存、Android ADB 扫描、自定义本地缓存路径; 扫描后选择卡片、加入任务、导出; 使用随包 FFmpeg 合并 m4s。\n"
...
def _调用大模型(提示词: str, 上下文: str) -> str:
    接口密钥: str = 工具.取接口密钥()
    模型: str = 工具.规范化接口模型(工具.取接口模型()) or "deepseek-v4-flash"
    if not 接口密钥:
...
def 流式对话SSE(提示词: str, 直接说: bool = False) -> 生成器[str]:
    ...
    try:
        ...
        上下文: str = _取项目上下文()
        _记录日志("info", f"正在流式调用大模型 ({工具.取接口模型() or 'deepseek-v4-flash'})...")
...
def 对话(提示词: str, 直接说: bool = False) -> dict:
    ...
    try:
        _记录日志("info", "正在获取项目代码上下文...")
        上下文: str = _取项目上下文()
        _记录日志("info", f"正在调用大模型 ({工具.取接口模型() or 'deepseek-v4-flash'})...")
        回复 = _调用大模型(提示词, 上下文)
